...
public class RepositoryDaoImpl implements RepositoryDao{
    ...
    @Override
    public List<String> getContributors_Extern(String repo, String token) throws IOException {
        JsonFactory f = new JsonFactory();
        ObjectMapper mapper = new ObjectMapper();

        List<String> result = new ArrayList<>();

        String contri = HttpRequest.getGithubContentUsingHttpClient(token,"api.github.com/repos/" + repo + "/contributors");
    ...
    public Pager<Repository> searchRepository(String name) {
        Map<String, Object> map = createMap();
        List<Repository> data = null;
        if (name.contains(":")) {
            String[] list = name.split(":");
            int index = Statistics.getPerformIndex(list[0]+":"+list[1].charAt(0));
            int balance = Integer.parseInt(list[1].substring(1));
            String methodName = searchMethod[index];
            Class clazz = this.getClass();
            try {
                Method method = clazz.getMethod(methodName, int.class);
                data = (List<Repository>) method.invoke(this, balance);
            ...
            data = mapper.searchRepository(map);
        }
    ...
    public Pager<Repository> getReposByLan_Key_Year(String language, String keyword, String year, String sort) {
        ...
        Map<String, Object> map = createMap();
        List<Repository> data = null;
        if(sort.equals("contributors")){
            data = mapper.selectReposByLan_Key_YearSortByContri(language,keyword,year,(int) map.get("pageSize"), (int) map.get("pageOffset"));
        }else {
            data = mapper.selectReposByLan_Key_Year(language, keyword, year, sort, (int) map.get("pageSize"), (int) map.get("pageOffset"));
        }
        Pager<Repository> page = createPage(data, map);
    ...
    public List<Repository> getStarLarger(int balance) {
        return mapper.getStarLarger(balance);
    }

    public List<Repository> getStarFewer(int balance) {
        return mapper.getStarFewer(balance);
    }

    public List<Repository> getForkLarger(int balance) {
        return mapper.getForkLarger(balance);
    }

    public List<Repository> getForkFewer(int balance) {
        return mapper.getForkFewer(balance);
    }

    public List<Repository> getContributeLarger(int balance) {
        return mapper.getContributeLarger(balance);
    }

    public List<Repository> getContributeFewer(int balance) {
        return mapper.getContributeFewer(balance);
    }
